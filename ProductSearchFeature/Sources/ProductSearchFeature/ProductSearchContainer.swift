...
public struct ProductSearchDIContainerImpl: ProductSearchDIContainerProtocol {
    ...
    public init(getImageUseCase: GetImageUseCaseProtocol) {
        ...
        networkService = NetworkServiceDataSourceImpl(baseURL: URL(string: "https://dummyjson.com")!)
        userDefaultsDataSource = UserDefaultsDataSource()

        // Initialize Repositories
        productSearchRepository = ProductSearchRepository(networkService: networkService)
        recentSearchesRepository = RecentSearchesRepository(userDefaultsDataSource: userDefaultsDataSource)
    }
